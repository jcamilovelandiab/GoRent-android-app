        ...
        if(InternetConnectivity.check(getContext())){
            ...
            itemListResultFB.observeForever(itemListQueryResultFirebase -> {
                ...
                if(itemListQueryResultFirebase.getItems()!=null){
                    List<Item> itemAvailableList = new ArrayList<>();
                    for(Item item : itemListQueryResultFirebase.getItems()){
                        if(!item.isDeleted()){
                            itemAvailableList.add(item);
                        }
                    }
                    ...
                    itemListQueryResult.setValue(new ItemListQueryResult(itemAvailableList));
                }
            });
